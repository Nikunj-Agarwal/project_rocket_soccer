...
learning strategy network (StrikeNet) to estimate optimal interception
timing and position with an online, shrinking-horizon Non-linear Model
Predictive Control (NMPC) solver for precise kinematic execution. By
...
over 50 randomized scenarios demonstrate an 88\% goal-scoring success
rate, with average strike position error of 0.349\,m and heading error
of 0.110\,rad.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Online Loop}: During live execution, StrikeNet
      infers the strategy from the current scene state in a single
      forward pass ($<$1\,ms). The system analytically propagates the
      ball's bounce trajectory to the predicted time, performs a
      36-candidate angular sweep to compute the exact goal-scoring
      heading, applies a backward target offset, and feeds these
      deterministic coordinates to an NMPC solver. The NMPC drives
      the car along a kinematically feasible trajectory. Upon contact
      (distance $<$\,0.35\,m), an elastic collision physics model
      computes the post-strike ball velocity, and a Phase~2 braking
      controller brings the vehicle to a halt.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Scoring Heading Sweep}: For each reachable
      ...
      propagated for up to 5.0\,s to check goal crossing. The first
      $(T, \theta_{\text{strike}})$ pair that satisfies both
      reachability and scoring is recorded as the training label.
\end{enumerate}
...
\textbf{Input Normalization.} The network applies z-score
normalization to inputs using per-feature mean and standard deviation
computed from the training split. These statistics are stored as
PyTorch registered buffers, ensuring they persist across model
save/load cycles.

\textbf{Training Details.} The model is trained using the Adam
optimizer ($\text{lr} = 10^{-3}$) with Mean Squared Error (MSE) loss
across all 5 output dimensions. An 80/20 train/test split is used with
a batch size of 256. Early stopping with patience of 20 epochs
prevents overfitting, and the best model (by test loss) is
...
restitution. A secondary case handles the car ``pushing'' the ball
forward when the car is moving faster than the ball in the normal
direction:
\begin{equation}
  \mathbf{v}_{\text{ball}}^{\text{post}} =
    \mathbf{v}_{\text{ball}} +
    (v_{\text{car}} - \mathbf{v}_{\text{ball}} \cdot \mathbf{n})
    (1 + e_{\text{strike}})\, \mathbf{n}
  \label{eq:push}
\end{equation}
To prevent steering oscillations and early contact triggers, we employ
...
backwards along the approach vector:
\begin{equation}
  \begin{aligned}
    x_{target} &= x_s - d_{\text{offset}} \cos(\theta_s) \\
    y_{target} &= y_s - d_{\text{offset}} \sin(\theta_s)
  \end{aligned}
  \label{eq:offset}
\end{equation}
...
\textbf{Solver Performance.} By suppressing IPOPT console output
(\texttt{print\_level:\,0}, \texttt{sb:\,"yes"}) and CasADi's I/O
overhead (\texttt{print\_time:\,False}, \texttt{verbose:\,False}), the
system achieved a 120$\times$ execution speedup. The NMPC solver
averages approximately 20.8\,ms per step (median: 16.2\,ms), with a
maximum single-step latency of 178.6\,ms and a maximum of 300
...
continue indefinitely at its impact speed.
...
Goal Success Rate & $\geq 60\%$ & \textbf{88\%} (44/50) \\
Avg Strike Pos.\ Error & $\leq 0.35$\,m & \textbf{0.349\,m} \\
Avg Strike Heading Error & $\leq 0.25$\,rad & \textbf{0.110\,rad} ($\approx 6.3^{\circ}$) \\
On-field Safety & 100\% & \textbf{100\%} \\
...
entirely, achieving 100\% IPOPT convergence across all 50 runs. The 6
failed scenarios (12\%) correspond to seeds where the exact scoring
heading sweep at runtime fails to find a valid deflection angle
(typically when the ball bounces into a corner far from the goal),
causing the system to fall back to a line-of-sight heading that does
not reliably score.
...
\begin{table*}[t]
\centering
\caption{System Descriptive Statistics and Control Effort Summary ($N = 50$, seeds 100--149, 95\% CI with $\alpha = 0.05$)}
...
The 6 failed seeds (107, 110, 114, 117, 140, 147) exhibit distinct
failure modes:
\begin{itemize}
    \item \textbf{No valid heading found} (Seeds 107, 114, 140, 147):
      The 36-candidate runtime heading sweep fails to find a
      deflection angle that redirects the ball into the goal mouth.
      The system falls back to a line-of-sight heading toward the goal
      center, which does not reliably score due to the ball's
      post-collision trajectory missing the 2.0\,m goal aperture.
    \item \textbf{Extreme heading misalignment} (Seeds 110, 114):
      These seeds exhibit heading errors exceeding 2.5\,rad at closest
      approach, indicating that the StrikeNet-predicted interception
      point resulted in a near-opposing approach angle.
\end{itemize}
...
The average NMPC solve time was 20.8\,ms per step (median:
16.2\,ms), well within the 100\,ms simulation timestep, confirming
real-time feasibility. The 120$\times$ speedup from solver output
suppression reduced the total batch execution time from approximately
10 minutes to under 8 minutes for all 50 seeds. The maximum
single-step latency of 178.6\,ms occurred during early horizon steps
with large state deviations.
...
(1)~the 36-candidate angular sweep is discretized at $10^{\circ}$
resolution, which may miss optimal headings in edge cases;
(2)~the ball physics model does not include drag, spin, or
gravitational effects, limiting realism;
(3)~the neural network is trained on a fixed field geometry and goal
configuration, reducing transferability;
(4)~the system assumes perfect state observation with no sensor noise
or latency.
The 12\% failure rate is primarily attributable to the coarse angular
discretization in the runtime heading sweep, which could be improved
with a finer sweep or gradient-based heading optimization.

\textbf{Future Work.}
Potential extensions include:
(1)~replacing the discrete heading sweep with a differentiable scoring
objective optimized jointly with the NMPC;
(2)~introducing observation noise and Kalman filtering for state
estimation;
(3)~extending to multi-agent scenarios with opponents and teammates;
(4)~transferring the learned policy to physical hardware using
...
\begin{enumerate}
    \item A hybrid StrikeNet + NMPC architecture that achieves 88\%
      goal-scoring accuracy over 50 diverse randomized scenarios.
    \item A pursuit-based warm-start strategy that guarantees 100\%
      solver convergence with an average solve time of 20.8\,ms.
    \item A target-offset heuristic that reduces heading error at
      contact from 0.61\,rad to 0.03\,rad.
