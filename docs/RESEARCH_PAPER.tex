...
% DOC PLACEHOLDERS (see docs/README.md):
%   PREVIOUS_INTEGRATION_BATCH  e.g. 20260612_155705 (previous try, hybrid+legacy)
%   LATEST_INTEGRATION_BATCH    newest data/tests/integration/ folder
%   LATEST_COMPARISON_RUN       newest data/tests/comparison/ folder
% Resolve: python -c "from src.data_layout import latest_integration_batch; ..."
...
robotic soccer striker. Offline imitation learning trains two StrikeNet
variants: \textbf{legacy} (predicts $T$, strike position, and heading) and
\textbf{structured} (predicts $T$ and heading; derives position from ball
physics). Online execution supports three planner modes---analytic,
neural, and hybrid (scoring-guard fallback)---enabling controlled ablation
via a five-config comparison harness. A shrinking-horizon NMPC solver
executes the plan with elastic collisions, target-offset heuristics,
pursuit warm-starting, and post-strike braking. Integration tests (100
seeds, 100--199) use strike-gated success (\texttt{scored AND
ball\_struck}). Reference run \texttt{20260613\_025809} (100 seeds):
analytic \textbf{80\%} success at ${\sim}561$\,ms median latency; pure neural
\textbf{44--46\%} at ${\sim}0.4$\,ms; hybrid \textbf{72--73\%} with bimodal
latency (${\sim}0.9$\,ms network path, ${\sim}8$\,ms fallback heading sweep---not
full analytic search). Structured variant: near-zero position error but no
success gain vs legacy hybrid.
\end{abstract}
...
This paper implements a two-phase hybrid approach: StrikeNet (legacy or
structured variant) dictates macroscopic strategy; a shrinking-horizon
NMPC solver manages microscopic execution. The \textbf{previous try}
exposed only hybrid mode with a legacy network. The \textbf{current system}
adds selectable \texttt{analytic}, \texttt{neural}, and \texttt{hybrid} modes,
two model variants, a five-config comparison harness, and cost/benefit analysis
(\texttt{scripts/analyze\_comparison.py}). We validate over 100 randomized seeds
(100--199).
...
0.61\,rad; with it, heading error drops to a median of essentially
0\,rad (mean 0.070\,rad over the batch).
...
initialization. In practice, the pursuit warm-start kept 48 of 50 test
runs entirely free of solver failures, with only 2 runs triggering
recoverable IPOPT restoration phases.
...
\textbf{100} randomized, distinct seeds (default: seeds 100--199).
Success requires both car--ball contact and a goal
(\texttt{success = scored AND ball\_struck}). The pass criterion is
strike-gated success rate $\geq 60\%$; the former closest-approach
position threshold is no longer a pass gate.

\subsection{Aggregate Performance}

\begin{table}[H]
\centering
\caption{Five-config comparison (seeds 100--199, run \texttt{20260613\_025809}; replace from \texttt{comparison.csv} after each pipeline run).}
\label{tab:results}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Config} & \textbf{Success} & \textbf{Pred err (m)} & \textbf{Med.\ lat.\ (ms)} \\ \midrule
analytic & 80\% & 0.064 & 561 \\
neural\_legacy & 46\% & 0.185 & 0.36 \\
neural\_structured & 44\% & 0.035 & 0.56 \\
hybrid\_legacy & 73\% & 0.137 & 0.88 \\
hybrid\_structured & 72\% & 0.048 & 0.91 \\
\bottomrule
\end{tabular}
\end{table}

Hybrid fallback share ${\sim}42$--49\%; fallback path median ${\sim}8$\,ms
(36-heading sweep at network $T$, not full \texttt{analytic\_strike\_plan}).
Pure neural modes fail the ${\ge}60\%$ deployment bar despite sub-ms latency.
...
\textit{Note: Tables~\ref{tab:descriptive}--\ref{tab:correlation} and
batch-specific figures below are templates from earlier 50-seed batches.
Regenerate via \texttt{scripts/analyze\_results.py --batch <id>} on the
latest 100-seed integration run.}

\begin{table*}[t]
\centering
\caption{System Descriptive Statistics and Control Effort Summary (template from a prior 50-seed batch; regenerate on latest 100-seed batch).}
...
\begin{figure}[H]
    ...
    \caption{NMPC solver performance metrics across the integration batch,
...
\textbf{Deployed latency} is \texttt{decision\_latency\_ms}: wall-clock of
\texttt{decide\_strike\_target()} (30-rep median for neural/hybrid), including
inference, ball rollout, scoring checks, and hybrid fallback sweep when it
fires. Diagnostic keys \texttt{strikenet\_infer\_ms} and
\texttt{analytic\_strategy\_ms} support scalability plots only.

Hybrid fallback (${\sim}8$\,ms when fired) is a 36-heading sweep at the
network's predicted time and ball position---\textbf{not} full
\texttt{analytic\_strike\_plan} (${\sim}560$\,ms: $T$-grid $\times$ reachability
$\times$ heading search). Report bimodal hybrid latency separately when fallback
share remains ${\sim}40$--50\%.
...
\begin{figure}[H]
    ...
    across the integration batch, confirming that the majority of runs achieve
...
\begin{figure}[H]
    ...
    integration seeds, showing the velocity-heading state-space trajectories.}
...
\begin{figure}[H]
    ...
    car-ball contact points across the integration batch on the
    10\,m~$\times$~6\,m field.}
...
\begin{enumerate}
    \item A hybrid StrikeNet + NMPC architecture with three planner modes,
      two model variants, and a five-config comparison harness;
      strike-gated success ($\geq 60\%$); previous try: 74\% over 50 seeds
      (\texttt{PREVIOUS\_INTEGRATION\_BATCH}); current metrics from
      \texttt{LATEST\_COMPARISON\_RUN}.
    \item A pursuit-based warm-start strategy that keeps solver failures
      rare (48/50 failure-free in the example batch).
    \item A target-offset heuristic that reduces heading error at
      contact from 0.61\,rad to a median of 0\,rad (mean 0.070\,rad).
