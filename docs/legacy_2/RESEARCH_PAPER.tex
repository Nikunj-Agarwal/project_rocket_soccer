...
system overcomes real-time kinodynamic constraints. Integration tests (default: 100 seeds, 100--199) use
strike-gated success (\texttt{scored AND ball\_struck}). In a
representative 50-seed run (batch \texttt{20260612\_155705}), the system
achieved 74\% success with the network driving execution in 52\% of
episodes; headline interception accuracy is
\texttt{strike\_point\_pred\_err\_m} (predicted target vs.\ ball at
contact).
\end{abstract}
...
the interception time, strike position, and strike heading from an
initial scene configuration), while a shrinking-horizon NMPC solver
manages microscopic execution. At runtime there is a single hybrid mode
(network prediction with scoring-guard analytic fallback;
\texttt{target\_source} logged per episode). We validate over 100
randomized integration test seeds (default: 100--199) spanning diverse
approach angles, multi-bounce trajectories, and challenging headings.
...
\begin{table}[H]
\centering
\caption{Primary Integration Test Results (illustrative $N = 50$ batch \texttt{20260612\_155705}; default suite is 100 seeds)}
\label{tab:results}
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Metric} & \textbf{Target} & \textbf{Achieved} \\ \midrule
Strike-gated Success Rate & $\geq 60\%$ & \textbf{74\%} (37/50) \\
Mean \texttt{strike\_point\_pred\_err\_m} & report & \textit{regenerate} \\
Solver Convergence & --- & \textbf{48/50} runs failure-free \\
\bottomrule
\end{tabular}
\end{table}

The pursuit-based warm-start kept 48 of 50 runs entirely free of solver
failures. In 26 of 50 episodes StrikeNet's plan passed the scoring
rollout (\texttt{target\_source = network}), scoring 16/26 (61.5\%);
in 24 fallback episodes, 21 scored (87.5\%). Regenerate this table on
the latest 100-seed batch via \texttt{scripts/analyze\_fallback.py}.
...
\begin{enumerate}
    \item A hybrid StrikeNet + NMPC architecture (single runtime mode;
      \texttt{target\_source} logged per episode) with strike-gated
      success ($\geq 60\%$ pass threshold over 100 default seeds;
      example: 74\% over 50 seeds in batch \texttt{20260612\_155705}).
